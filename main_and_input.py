...
if __name__ == "__main__":
    ...
    print_rating(WSM(norm_df, weights_wsm), Max)
    ...
    print_rating(PC(norm_df, weights_pc), Max)

    print("\nМЕТОД МУЛЬТИПЛИКАТИВНОЙ СВЕРТКИ")
    weights_mc = weights_wsm  # Веса метода мультипликативной свертки
    print_rating(MC(norm_df, weights_mc), Max)

    print("\nМЕТОД ИДЕАЛЬНЫХ ТОЧЕК")
    ideal_point_str = [2700, 30, 600, 2.5, 215, 36, 24]  # Идеальная точка
    print_rating(ideal_point(norm_df, ideal_point_str, direction_optimisation, extramural), Min)

    print("\nМетод Гермейера")
    weights_bundle = weights_wsm  # Веса свертки Гермейера
    print_rating(hermeyer_bundle(norm_df, weights_bundle), Max)

    print("\nОбласть Паретто")
    k1 = dict(zip(input_data['Оружие'], input_data['Стоимость ($)']))
    k2 = dict(zip(input_data['Оружие'], input_data['Урон']))
    parret_set(k1, Min, 'Стоимость ($)', k2, Max, 'Урон')
